...
def font_generator_model():
    ...
    model.add(Dense(input_dim=100, output_dim=512*5*5))
    # model.add(BatchNormalization())
    model.add(Activation('relu'))

    model.add(Reshape((512, 5, 5)))
    model.add(UpSampling2D(size=(2, 2)))
...
def font_discriminator_model():
    ...
    model.add(Convolution2D(64, 5, 5, subsample=(2,2), border_mode='same', input_shape=(1, 64, 64)))
    ...
    for epoch in range(100):
        ...
        for index in range(int(org_imgs.shape[0] / batch_size)):
            ...
            if index % 20 == 0:
                ...
                img = img*127.5+127.5
                Image.fromarray(img.astype(np.uint8)).save(str(epoch)+"_"+str(index)+".png")
